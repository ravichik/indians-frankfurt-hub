...
const SITE_NAME = 'Indians in Frankfurt Hub';
...
export const shareOptions = {
  ...
  email: {
    ...
    icon: '✉️',
    color: '#EA4335',
    getUrl: (url, text, title) => `mailto:?subject=${encodeURIComponent(title || 'Check this out from Indians in Frankfurt')}&body=${encodeURIComponent(text + '\n\n' + url)}`
  },
  copy: {
    name: 'Copy Link',
    icon: '🔗',
...
export const getResourceShareData = (resource) => {
  ...
  const text = `${title} - Useful resource for Indians in Frankfurt`;
